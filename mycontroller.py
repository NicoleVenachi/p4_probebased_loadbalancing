...
def main(p4info_file_path_sw, bmv2_file_path_sw, p4info_file_path_tor, bmv2_file_path_tor):
    ...
    try:
        ...
        stor11 = p4runtime_lib.bmv2.Bmv2SwitchConnection(
            name='stor11',
            address='127.0.0.1:50059',
            device_id=8,
            proto_dump_file='logs/stor11-p4runtime-requests.txt')
        stor22 = p4runtime_lib.bmv2.Bmv2SwitchConnection(
            name='stor22',
            address='127.0.0.1:50060',
            device_id=9,
            proto_dump_file='logs/stor22-p4runtime-requests.txt')
        ...
        stor11.MasterArbitrationUpdate()
        stor22.MasterArbitrationUpdate()
        ...
        s11.SetForwardingPipelineConfig(p4info=p4info_helper_sw.p4info,
                                       bmv2_json_file_path=bmv2_file_path_sw)
        print "Installed P4 Program using SetForwardingPipelineConfig on s11"
        s22.SetForwardingPipelineConfig(p4info=p4info_helper_sw.p4info,
                                       bmv2_json_file_path=bmv2_file_path_sw)
        print "Installed P4 Program using SetForwardingPipelineConfig on s22"
        ...
        stor11.SetForwardingPipelineConfig(p4info=p4info_helper_tor.p4info,
                                       bmv2_json_file_path=bmv2_file_path_tor)
        print "Installed P4 Program using SetForwardingPipelineConfig on stor11"
        stor22.SetForwardingPipelineConfig(p4info=p4info_helper_tor.p4info,
                                       bmv2_json_file_path=bmv2_file_path_tor)
        print "Installed P4 Program using SetForwardingPipelineConfig on stor22"
        ...
        with open("s11-runtime.json", 'r') as sw_conf_file:
            sw_conf = json_load_byteified(sw_conf_file)
            if 'table_entries' in sw_conf:
                table_entries = sw_conf['table_entries']
                info("Inserting %d table entries..." % len(table_entries))
                for entry in table_entries:
                    info(tableEntryToString(entry))
                    insertTableEntry(s11, entry, p4info_helper_sw)
        with open("s22-runtime.json", 'r') as sw_conf_file:
            sw_conf = json_load_byteified(sw_conf_file)
            if 'table_entries' in sw_conf:
                table_entries = sw_conf['table_entries']
                info("Inserting %d table entries..." % len(table_entries))
                for entry in table_entries:
                    info(tableEntryToString(entry))
                    insertTableEntry(s22, entry, p4info_helper_sw)
        with open("tor11-runtime.json", 'r') as sw_conf_file:
            sw_conf = json_load_byteified(sw_conf_file)
            if 'table_entries' in sw_conf:
                table_entries = sw_conf['table_entries']
                info("Inserting %d table entries..." % len(table_entries))
                for entry in table_entries:
                    info(tableEntryToString(entry))
                    insertTableEntry(stor11, entry, p4info_helper_tor)
        with open("tor22-runtime.json", 'r') as sw_conf_file:
            sw_conf = json_load_byteified(sw_conf_file)
            if 'table_entries' in sw_conf:
                table_entries = sw_conf['table_entries']
                info("Inserting %d table entries..." % len(table_entries))
                for entry in table_entries:
                    info(tableEntryToString(entry))
                    insertTableEntry(stor22, entry, p4info_helper_tor)
